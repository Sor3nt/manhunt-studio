...
export default class Ifp extends AbstractLoader{
    ...
    static list(binary, options){
        ...
        switch (fourCC) {
            ...
            case Ifp.FOURCC_ANCT:
                let numBlock = binary.consume(4, 'int32');

                for (let i = 0; i < numBlock; i++) {
                    binary.seek(4);
                    let bNameLen = binary.consume(4, 'int32');
    ...
    /**
     * @param binary {NBinary}
     * @returns {{frameTimeCount: [], anpkName: [], anpkOffset: []}}
     */
    ...
    static readStrmAnimBinIndex(binary) {
        ...
        for (i = 0; i < numExec; i++) {
            ...
            binary.seek(4); //JumpExectuionSize
        ...
        for(let i in bones){
            ...
            bone.frames[0].frames.forEach(function (frame, i) {
                ...
                if (frame.quat.length > 0){
                    ...
                    if (convertNames){
                        if (name === "Bip01_Spine"){
                            vec4.multiply(new Quaternion(-0.500398, -0.500001, 0.4996, 0.500001));
